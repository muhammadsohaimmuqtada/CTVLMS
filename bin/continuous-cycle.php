...
$db=getDB();
$summary=[
    'started_at'=>gmdate(DATE_ATOM),'nvd'=>['ok'=>false,'processed'=>0],
    'local_inventory'=>[],'ssh_inventory'=>[],'scans'=>[],
    'correlation'=>null,'package_correlation'=>null,'jobs_queued'=>0,'patches'=>[],'verification'=>null,
];
...
foreach (configuredAssetIDs('CTVLMS_LOCAL_ASSET_IDS') as $entry) {
    ...
    $result=runChild([PHP_BINARY,__DIR__.'/inventory-local.php',(string)$entry['id']]);
    $summary['local_inventory'][]=['asset_id'=>$entry['id'],'ok'=>$result['exit']===0,'output'=>$result['exit']===0?$result['stdout']:$result['stderr']];
}
...
foreach (configuredAssetIDs('CTVLMS_SSH_INVENTORY_ASSET_IDS') as $entry) {
    ...
    $result=runChild([PHP_BINARY,__DIR__.'/inventory-ssh.php',(string)$entry['id']]);
    $summary['ssh_inventory'][]=['asset_id'=>$entry['id'],'ok'=>$result['exit']===0,'output'=>$result['exit']===0?$result['stdout']:$result['stderr']];
}
